...
def print_snapshot(snapshot: PerformanceSnapshot) -> None:
    ...
    print(f"\n--- {snapshot.timestamp.strftime('%H:%M:%S')} ---")  # noqa: T201
    print(f"Battery: {snapshot.battery_level}%")  # noqa: T201
    print(f"CPU: {snapshot.cpu_usage:.1f}%")  # noqa: T201
    print(f"Memory: {snapshot.memory_used_mb}/{snapshot.memory_total_mb} MB")  # noqa: T201
    print(f"Disk: {snapshot.disk_used_percent:.1f}%")  # noqa: T201
    print(f"Processes: {snapshot.running_processes}")  # noqa: T201
...
def main() -> None:
    ...
    if args.mode == "logcat":
        print("Starting logcat monitor (Ctrl+C to stop)...")  # noqa: T201
        ...
        print("Starting event monitor (Ctrl+C to stop)...")  # noqa: T201
        EventMonitor(args.serial).start_event_capture()
    elif args.mode == "crash":
        print("Starting crash monitor (Ctrl+C to stop)...")  # noqa: T201

        def on_crash(crash: CrashEvent) -> None:
            print("\n!!! CRASH DETECTED !!!")  # noqa: T201
            print(  # noqa: T201
                json.dumps(
                    {
